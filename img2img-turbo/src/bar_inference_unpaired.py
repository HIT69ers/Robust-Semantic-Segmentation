...
if __name__ == "__main__":
    ...
    print("\n-----Start-----")
    for index, input_image in enumerate(os.listdir(args.input_image_dir)):
        name = input_image
        input_image = Image.open(args.input_image_dir + '/' + input_image).convert('RGB')
        # translate the image
        with torch.no_grad():
        ...
        output_pil.save(os.path.join(args.output_dir, name))
        
        print('\r', end='')
        progress = (index + 1) / len(os.listdir(args.input_image_dir)) * 100
        bar = '▋' * int(progress // 2)
        print(f'Transform progress: {progress:.2f}%: {bar}', end='')
        sys.stdout.flush()
